 ...
 *
 * @param input_weights Pointer to the input weight matrix.
 * @param packed_weights Pointer to the output buffer where packed weights will be stored.
...
void tgemm_pack_weights(const int8_t* input_weights, uint8_t* packed_weights, int rows, int cols) {
    ...
    assert((cols % 4) == 0 && "Number of columns must be divisible by 4.");
        ...
        for (int col = 0; col < cols; col += 4) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (w == -1) {
                    ...
                    assert(false && "Input weight must be -1, 0, or 1");
                }
            ...
            packed_weights[packed_index++] = byte_val;
        }
    }
}
